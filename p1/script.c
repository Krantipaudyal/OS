...
#define MAX_LINE 80 //* maximum length of the commad 
	...
	int cmdLoop(void)
	{
		...
		while (continueLoop)
		{
		       	...
		       	else
			{
				loops++;
				if (isAmp == 0)
				{ //Wait for child process to terminate if '&' follows command
					loops++;
				       	wait(NULL);
			       	}
			}
